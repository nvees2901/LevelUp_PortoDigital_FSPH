  ...
  return (
    <div className="space-y-5 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        <h3 className="font-bold text-brand-primary text-sm mb-3">Distribuicao por Etapa do Fluxo COLIC</h3>
        <div className="flex flex-wrap gap-2">
          {ETAPAS.map((etapa, i) => {
            const count = stats.por_status[etapa] ?? 0;
            return (
              <div key={etapa}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all ${count > 0 ? 'bg-blue-50 border-blue-200 text-brand-primary' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
                <span className="text-slate-300 font-normal">{i + 1}.</span> {etapa}
                {count > 0 && (
                  <span className="bg-brand-primary text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold">{count}</span>
                )}
              </div>
            );
          })}
            ...
            {pendentes.map(term => (
              <div key={term.id} className="p-4 hover:bg-slate-50 flex items-center justify-between gap-4 transition-colors">
                ...
                <button onClick={() => navegar('detalhe', term.id)}
                  className="px-4 py-2 bg-brand-primary text-white rounded-lg text-xs font-bold hover:bg-brand-hover transition shrink-0">
                  Analisar
                </button>
              </div>
            ))}
